...
const {
	UPLOADS_USERS_DIRECTORY,
	USERS_IMAGE_MAX_WIDTH,
	UPLOADS_ADMINS_DIRECTORY,
} = process.env;
const uploadsUsersDir = path.join(__dirname, '../.', UPLOADS_USERS_DIRECTORY);
const uploadsAdminDir = path.join(__dirname, '../.', UPLOADS_ADMINS_DIRECTORY);
...
async function saveAdminPhoto(imageBuffer) {
	await ensureDir(uploadsAdminDir);
	const image = sharp(imageBuffer.data);
	const imageInfo = await image.metadata();
	const maxSize = Number(USERS_IMAGE_MAX_WIDTH);
	...
	const savedImageName = `${uuid.v4()}.jpg`;
	const imagePath = path.join(uploadsAdminDir, savedImageName);
	await image.toFile(imagePath);
	return savedImageName;
}
...
async function removeAdminPhoto(photoName) {
	if (photoName) {
		const photoPath = path.join(uploadsAdminDir, photoName);
		await unlink(photoPath);
	}
}
