...
export default function IncidenceForm({
  ...
  async function performSubmit(e) {
    ...
    try {
      ...
      const route = `${host}/api/incidences/?id=${reservations[index].id}`;
  ...
  async function requestCleaning(e) {
    ...
    try {
      ...
      const route = `${host}/api/reserves/cleaning/?id=${reservations[index].id}`;
  ...
  return (
    <article className={`${className} reservesPresentation`}>
      ...
      >
        <p>Informa al anfitrión si quieres limpiar tu espacio reservado.</p>
        {error2 && <p className="form-error">{error2}</p>}
        {message2 && <p className="form-message">{message2}</p>}
