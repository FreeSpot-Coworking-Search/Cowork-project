...
export default function ListCentersSearch(searchObject) {
  const [data, setData] = useState(getSpaces(searchObject));

  useEffect(() => {
    setData(getSpaces(searchObject));
  }, [searchObject]);
...
const getSpaces = async (searchObject) => {
  try {
    const response = await axios.post(
      'http://localhost:3000/api/centers/search',
      searchObject
    );
